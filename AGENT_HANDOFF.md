...
2. the session is rebound to the next available account of the same provider in route order, then to the next available provider if that provider is exhausted;
...
If no configured account is available, the runtime enters an exhausted state with a clear detail.
...
Remote access now configures separate tailnet-only Serve proxies: dashboard :8443 -> 127.0.0.1:8787 and phone-app server :8444 -> 127.0.0.1:6767. The phone must connect to the same tailnet; access follows tailnet ACLs. Serve approval URLs from timed-out CLI output are validated and displayed, errors persist, and disabled proxies have no copyable URL. User approval of network Serve/HTTPS settings is required on first use.

The active `https://<tailnet-host>:8444` server origin is stored in `~/.omnigent/omni-route-trusted-origins.json` with mode `0600`. The patched Omnigent HTTP and WebSocket origin policy reads that file dynamically. Remote access enable/disable synchronizes the file, and `install.sh` synchronizes it and restarts the patched daemon so replacing the runtime cannot leave an old zygote or lose the trusted phone origin.

Claude workspace trust is profile-scoped. Before launching Claude, orchestration writes `hasTrustDialogAccepted` for the resolved workspace into the selected account's isolated `<CLAUDE_CONFIG_DIR>/.claude.json` using an atomic mode-`0600` update. This prevents Claude Code's interactive trust prompt from immediately terminating an Omnigent terminal stream while preserving every other profile setting.

Every isolated Claude profile links missing native skill entries from `~/.agents/skills` into `<CLAUDE_CONFIG_DIR>/skills` before launch. Existing account-specific entries are preserved. This keeps native slash workflows such as `/implement` available across all pooled Claude subscriptions in addition to the Omnigent MCP `load_skill` catalog.

The patched Claude native bridge verifies drafts across every wrapped row of the live composer, stopping at its lower box rule. If paste verification still times out, it clears the unsubmitted composer before returning an error. This prevents narrow terminal panes from rejecting visible multiline prompts and accumulating duplicate drafts on retry.
