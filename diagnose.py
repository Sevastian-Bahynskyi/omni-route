...
def identity_from_auth(path: Path) -> dict[str, str | None]:
    ...
    account_id = find_string(data, {"account_id", "accountid", "chatgpt_account_id", "chatgptaccountid", "workspace_id", "workspaceid"})
    subject = None
    tokens: list[str] = []
    ...
    def collect_tokens(value: object) -> None:
        if isinstance(value, dict):
            for key, item in value.items():
                if isinstance(item, str) and str(key).casefold().replace("-", "_") in {"id_token", "access_token"}:
                    tokens.append(item)
                else:
                    collect_tokens(item)
        elif isinstance(value, list):
    ...
    for token in tokens:
        ...
        account_id = account_id or find_string(payload, {"account_id", "accountid", "chatgpt_account_id", "chatgptaccountid", "workspace_id", "workspaceid"})
        if subject is None and isinstance(payload.get("sub"), str):
            subject = payload["sub"]
    return {"email": email, "account_id": account_id, "subject": subject}


def run(args: list[str], *, env: dict[str, str] | None = None, timeout: float = 10.0) -> subprocess.CompletedProcess[str] | None:
    try:
        return subprocess.run(args, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL, timeout=timeout, check=False)
...
with tempfile.TemporaryDirectory() as td:
    ...
    pool=CodexAccountPool(PoolConfig(accounts=(AccountProfile("a",a),AccountProfile("b",b))), state_path=td/"state.json", now=lambda:1000)
...
def test_wiring(rep: Reporter) -> None:
    checks = [
        ...
        (SRC / "omnigent" / "claude_native_bridge.py", "composer_lines", "wrapped Claude draft delivery"),
    ...
    for path, needle, label in checks:
        try:
            ...
            continue
        rep.emit(PASS if needle in text else FAIL, f"runtime wiring: {label}", "" if needle in text else f"marker {needle!r} missing")


def test_live_bridge(rep: Reporter, configured_names: set[str]) -> None:
    if not BRIDGE_ROOT.is_dir():
        rep.emit(WARN, "live Codex bridge", "no bridge directory yet; start `omni-rotate start`")
        return
    bridge_dirs = sorted({p.parent for p in BRIDGE_ROOT.glob("*/state.json")}, key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True)
    if not bridge_dirs:
...
async def main():
    bridge=Path(sys.argv[1]); state=read_bridge_state(bridge)
    if state is None: raise RuntimeError("bridge state unreadable")
...
'''
    ...
    try:
        ...
        return
    account = payload.get("account"); quota = payload.get("quota")
    rep.emit(PASS if isinstance(account, dict) and "result" in account else FAIL, "live account/read", "Codex app-server responded" if isinstance(account, dict) and "result" in account else "unexpected response shape")
    if isinstance(quota, dict) and "result" in quota:
        detail = "Codex app-server responded"; result_obj = quota.get("result")
        if isinstance(result_obj, dict):
            ...
            if isinstance(rl, dict):
                ...
                if percents: detail += " // " + ", ".join(percents)
...
def main() -> int:
    ...
    if SRC.is_dir() and VENV_PYTHON.is_file() and PATCHED_OMNI.is_file():
        rep.emit(PASS, "patched Omnigent runtime", str(SRC)); version = run([str(PATCHED_OMNI), "--version"], timeout=8)
        if version and version.returncode == 0:
            text = (version.stdout or version.stderr).strip().splitlines(); rep.emit(PASS, "patched omni executable", text[-1] if text else "responded")
        else: rep.emit(FAIL, "patched omni executable", "could not run --version")
    else: rep.emit(FAIL, "patched Omnigent runtime", "installation incomplete")
    test_wiring(rep); test_synthetic_rotation(rep)
    config = read_json(CONFIG_PATH)
    if not config:
        rep.emit(FAIL, "account pool config", f"missing/unreadable {CONFIG_PATH}"); return rep.summary()
    ...
    if STATE_PATH.exists() and not state: rep.emit(FAIL,"pool state","exists but is unreadable")
        ...
        if current is None: rep.emit(INFO,"current account","none selected yet")
        elif isinstance(current,str) and current in configured_names: rep.emit(PASS,"current account",current)
        else: rep.emit(FAIL,"current account",f"unknown value {current!r}")
        ...
        if isinstance(cooldowns,dict):
            ...
            for name,value in cooldowns.items():
                if isinstance(value,dict) and isinstance(value.get("retry_at"),(int,float)):
                    ...
                    if retry>now: active.append(f"{name} until {time.strftime('%H:%M:%S',time.localtime(retry))}")
